...
class SettingPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // SettingsProvider is assumed to be provided at app root and already loaded.
...
final _currencyOpts = const ['INR (₹)', 'USD (\$)', 'EUR (€)'];
  ...
  // Numbering controllers (persisted in DB)
  final _poCtrl = TextEditingController();
  final _soCtrl = TextEditingController();
  final _invCtrl = TextEditingController();
  final _quoCtrl = TextEditingController();
  bool _resetCounters = false;

  bool _seeded = false; // basic org seeding
  bool _seededNum = false; // numbering/app seeding
  bool _dirty = false;

  // Onboarding locations
  final List<_LocationCtrls> _locations = [_LocationCtrls()];
String _ensureInOptions(String? v, List<String> opts, String fallback) {
  if (v == null) return fallback;
  return opts.contains(v) ? v : fallback;
}
@override
void didChangeDependencies() {
  ...
  if (p.loaded && p.onboarded && !_seededNum) {
    // Fallback to first valid option if stored value is not present
    _currency = _ensureInOptions(p.app.currency, _currencyOpts, _currencyOpts.first);
    _dateFmt  = _ensureInOptions(p.app.dateFormat, _dateFmtOpts, _dateFmtOpts.first);
    _gstPct   = _ensureInOptions('${p.app.defaultGstPct ?? 18}%', _gstOpts, '18%');

    _poCtrl.text  = p.numbering.po;
    _soCtrl.text  = p.numbering.so;
    _invCtrl.text = p.numbering.inv;
    _quoCtrl.text = p.numbering.quo;
            ...
            children: [
              ...
              Expanded(
                child: TabBarView(
                  ...
                  children: [
                    _orgTab(context), // Organization tab
                    _numberingTab(),
                    _appSettingsTab(),
                    _usersTab(),
                  ],
              ...
              Row(
                children: [
                  ...
                  OutlinedButton.icon(
                    ...
                    onPressed: () =>
                        setState(() => _locations.add(_LocationCtrls())),
                  ...
                  Expanded(
                    child: FilledButton(
                      ...
                      onPressed: () async {
                        ...
                        // Collect onboarding locations
                        final locs = <LocationInput>[];
                        for (final l in _locations) {
                          locs.add(LocationInput(
                            name: l.name.text.trim().isEmpty
                                ? 'Warehouse ${locs.length + 1}'
                                : l.name.text.trim(),
                            address: l.address.text.trim(),
                            city: l.city.text.trim(),
                            state: l.state.text.trim(),
                            pincode: l.pincode.text.trim(),
                            isActive: locs.isEmpty, // first active by default
                          ));
                        }
      ...
      child: Column(
        ...
        children: [
          ...
          twoCol(
            TextFormField(
              controller: l.name,
              decoration: _soft('Warehouse Name'),
            ),
  ...
  Widget _orgTab(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        // Warehouses (DB-backed)
        _SectionCard(
          header: _CardHeader(
            title: 'Warehouses',
            trailing: FilledButton.icon(
              onPressed: () async {
                final r = await _openWarehouseDialog();
                if (r != null) {
                  await context.read<SettingsProvider>().addWarehouse(
                        name: r.name,
                        address: r.address,
                        isActive: r.enabled,
                      );
          ...
          child: _WarehousesTable(
            rows: p.warehouses
                .map((w) =>
                    _WhRow(w.name, w.address, w.isActive, id: w.id))
                .toList(),
            onToggle: (i, v) async {
              final id = p.warehouses[i].id;
              await context.read<SettingsProvider>().setWarehouseActive(id, v);
            },
            onDelete: (i) async {
              final id = p.warehouses[i].id;
              await context.read<SettingsProvider>().deleteWarehouse(id);
            },
            onEdit: (i) async {
              final w = p.warehouses[i];
              final r = await _openWarehouseDialog(
                initial: _WhRow(w.name, w.address, w.isActive, id: w.id),
              );
              if (r != null) {
                await context.read<SettingsProvider>().updateWarehouse(
                      id: w.id,
                      name: r.name,
                      address: r.address,
                      isActive: r.enabled,
            ...
            children: [
              ...
              SizedBox(
                ...
                child: FilledButton(
                  ...
                  onPressed: () async {
                    await context.read<SettingsProvider>().saveNumbering(
                          po: _poCtrl.text.trim(),
                          so: _soCtrl.text.trim(),
                          inv: _invCtrl.text.trim(),
                          quo: _quoCtrl.text.trim(),
                          resetCounters: _resetCounters,
                        );
                    if (!mounted) return;
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content:
                              Text('Numbering settings saved')),
                    );
                  },
  ...
  Future<_WhRow?> _openWarehouseDialog({_WhRow? initial}) async {
    ...
    bool active = initial?.enabled ?? true;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
              flex: 4,
              child: Text('Address', style: TextStyle(color: grey))),
          SizedBox(
              width: 64,
